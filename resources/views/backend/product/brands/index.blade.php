				...
				<table class="table aiz-table mb-0">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-right">
							@if($brand->link_brand)
    <a href="{{ $brand->link_brand }}" target="_blank" title="Visit Website" class="btn btn-soft-primary btn-icon btn-circle btn-sm">
        <i class="las la-globe-americas text-info"></i>
    </a>
@endif
								@can('edit_brand')
		...
		<div class="card">
			...
			<div class="card-body">
				<form action="{{ route('brands.store') }}" method="POST">
					...
					<div class="form-group mb-3">
						<label for="name">{{translate('Link Brand')}}</label>
						<input type="text" class="form-control" name="link_brand" placeholder="{{translate('Link Brand')}}">
					</div>
					<div class="form-group mb-3">
